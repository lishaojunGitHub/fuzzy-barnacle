...
<section class="panel">
	<header class="panel-heading">
		...
		<form action="/admin/cp/cp/list" method="POST">
		...
		<a href="/admin/cp/cp/add" style="margin-left:10px;" class="btn btn-success" data-toggle="confirmation" data-original-title="" title="">添加</a>
	</header>
	<div class="panel-body table-responsive">
		<table class="table table-bordered mb-none table" role="grid" aria-describedby="datatable-default_info">
			...
			<tbody>
				...
				<tr class="gradeX" role="row">
					...
					<td style="width:130px">${bean.province}</td>
					...
					<td style="width:150px">
						...
						<a href="/admin/cp/cp/edit?id=${bean.id}" class="btn btn-primary  btn-xs">编辑</a>&nbsp;&nbsp;
		...
		<div class="col-sm-12 col-md-6">
			...
			<@html.pagination  obj_count=total  url="/admin/cp/cp/list" current_page=page page_size=pageSize/>
			</div>
		</div>
	</div>
</section>
...
function del(id) {
	if (window.confirm('你确定要删除吗')) {
		$.ajax({
			type: 'POST',
		    url: "/admin/cp/cp/delete?id="+id,
		    data: {} ,
		    success: function(data) {
		    	if(data.code > 0){
		    		alert(data.msg);
		    		window.location="/admin/cp/cp/list";
		    	} else if (data.code==0){
		    		alert(data.msg);
		    	}
		    },
